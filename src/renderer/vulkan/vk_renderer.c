...
static void create_frame_data(VK_Renderer *engine)
{
  ...
  // Create command buffer allocation
...
void VK_Draw(VK_Renderer *engine)
{
  ...
  vkWaitForFences(engine->device, 1, active_fence, VK_TRUE, UINT32_MAX);
  vkWaitForFences(engine->device, 1, active_fence, VK_TRUE, UINT32_MAX);
  ...
  transition_image(cmd, engine->swapchain_data.images[swapchain_image], VK_IMAGE_LAYOUT_UNDEFINED, VK_IMAGE_LAYOUT_GENERAL);
  // Rendering

  transition_image(cmd, engine->swapchain_data.images[swapchain_image], VK_IMAGE_LAYOUT_GENERAL, VK_IMAGE_LAYOUT_PRESENT_SRC_KHR);
  ...
  VkSemaphoreSubmitInfo signal_info = submit_info_semaphore(
      ...
      engine->frames_in_flight.render_semaphore[frame_index]);
...
static void create_frame_data(VK_Renderer *engine)
{
  ...
  for (int i = 0; i < engine->swapchain_data.image_count; i++)
  {
...
void create_default_pipeline(VK_Renderer *engine)
{
  ...
  VKShader shader = VKShader_create(
      engine->device,
      "resource/shaders/triangle.vert.spv",
      "resource/shaders/triangle.frag.spv");
...
void VK_Draw(VK_Renderer *engine)
{
  ...
  /* TODO: once VKPipeline_build gives you a VkPipeline, store it on the
   * engine (or pass it in) and bind + draw here, e.g.:
   *   vkCmdBindPipeline(cmd, VK_PIPELINE_BIND_POINT_GRAPHICS, engine->triangle_pipeline);
   *   vkCmdDraw(cmd, 3, 1, 0, 0);
   */
